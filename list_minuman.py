...
data_minuman = baca_csv("menu_minum.csv")

for i, item in enumerate(data_minuman):
    entry = buat_label_entry(scrollable_frame, item["Nama Menu"], i + 1, 0)
    button_pesan = ctk.CTkButton(master=scrollable_frame, text="Pesan", command=lambda item=item["Nama Menu"], jumlah=entry: pesan(item, jumlah))
