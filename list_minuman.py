...
def simpan_pesanan(pesanan):
    ...
    with open("order_minum.csv", "w", newline='', encoding='utf-8') as csvfile:  # Ganti nama file menjadi 'order_minum.csv'
    ...
    messagebox.showinfo("Pesanan Disimpan", "Pesanan telah disimpan ke file order_minum.csv")  # Ubah pesan dialog
...
def update_order_summary(pesanan):
    ...
    for menu, jumlah in pesanan.items():
        for item in data_minuman:  # Ganti 'data_makanan' menjadi 'data_minuman'
...
root = ctk.CTk()
root.title("Daftar Minuman")  # Ganti judul menjadi "Daftar Minuman"
...
data_minuman = baca_csv("menu_minum.csv")  # Ganti 'menu_makan.csv' menjadi 'menu_minum.csv'
...
for i, item in enumerate(data_minuman):  # Ganti 'data_makanan' menjadi 'data_minuman'
